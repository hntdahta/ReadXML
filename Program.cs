...
namespace ReadXML
{
    ...
    class ReadWriteFileXML
    {
        public static bool WriteToFile()
        {
            ...
            try
            {
                XDocument xDoc = XDocument.Load(@"C:\Users\hntdahta\source\repos\ReadXML\books.xml");
                XElement newBook = new XElement("book",
                new XElement("title", book.Title),
                new XElement("price", book.Price.ToString()));

                xDoc.Element("bookstore").Add(newBook);
                xDoc.Save(@"C:\Users\hntdahta\source\repos\ReadXML\books.xml");
                return true;
            }
            catch (Exception ex)
            {
        ...
        {
            XmlTextReader reader = new XmlTextReader(@"C:\Users\hntdahta\source\repos\ReadXML\books.xml");
            while (reader.Read())
            {
